...
export default function AulaPage() {
  ...
  if (loading) {
    return (<div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="w-12 h-12 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin"></div></div>);
  }

  if (!aula || !modulo) {
    return (<div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center"><p className="text-gray-500 mb-4">Aula não encontrada.</p><Link href={`/plataforma/enem/modulo/${moduloId}`} className="text-blue-600 hover:underline">Voltar</Link></div>);
  }
          ...
          {secaoAtiva === 'formulas' && aula.formulas && (
            <div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                <div className="text-gray-700 whitespace-pre-line font-mono text-sm">{aula.formulas}</div>
              </div>
            </div>
          )}
          {secaoAtiva === 'dicas' && aula.dicas && (
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-xl font-bold text-gray-900">Dicas</h2>
              </div>
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <div className="text-gray-700 whitespace-pre-line">{aula.dicas}</div>
              </div>
            </div>
          )}
