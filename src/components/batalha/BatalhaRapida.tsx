...
import { registrarBatalha } from '@/lib/xp-system';
...
export default function BatalhaRapidaPage() {
  ...
  const handleTempoEsgotado = () => {
    ...
    setTimeout(() => {
      if (questaoAtual < questoes.length - 1) {
        ...
        finalizarBatalha(novosResultados);
      }
  ...
  const finalizarBatalha = async (resultadosFinais: Resultado[]) => {
    setFinalizado(true);

    if (userId) {
      const acertos = resultadosFinais.filter(r => r.correta).length;
      const result = await registrarBatalha(supabase, userId, acertos, 5);
      if (result) {
        setXpResult(result);
  ...
  const calcularEstatisticas = () => {
    ...
    return { acertos, tempoTotal, tempoMedio };
  };
          ...
          <div className="bg-white rounded-3xl p-8 text-center shadow-2xl">
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                ...
                <p className="text-xs text-gray-400">XP máximo</p>
              </div>
            </div>
  ...
  // Tela de resultado
  if (finalizado) {
    const { acertos, tempoMedio } = calcularEstatisticas();
      ...
      <div className="min-h-screen bg-gradient-to-br from-amber-500 to-orange-600 p-4">
        {showXPNotification && xpResult && (
          <XPNotification
            ...
            subiuNivel={xpResult.subiuNivel}
            novoNivel={xpResult.nivelInfo.titulo}
          ...
          <div className="bg-white rounded-3xl p-8 text-center shadow-2xl mb-4">
            ...
            {xpResult && (
              <div className="bg-amber-50 rounded-xl p-4 mb-6">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <p className="text-2xl font-bold text-amber-600">+{xpResult.xpGanho} XP</p>
                </div>
                ...
                {xpResult.streak > 1 && (
                  <div className="flex items-center justify-center gap-1 mt-2 text-orange-500 text-sm">
                    ...
                    <span>{xpResult.streak} dias de streak!</span>
                  </div>
                )}
                <p className="text-amber-500 text-xs mt-2">XP Total: {xpResult.xpTotal}</p>
              </div>
            )}
            ...
            <div className="flex justify-center gap-2 mb-6">
              {resultados.map((r, i) => (
                <div 
                  key={i}
                  className={`w-10 h-10 rounded-lg flex items-center justify-center ${r.correta ? 'bg-emerald-100' : 'bg-red-100'}`}
                >
                  {r.correta ? <CheckCircle className="w-5 h-5 text-emerald-600" /> : <XCircle className="w-5 h-5 text-red-600" />}
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              ...
              <Link href="/dashboard" className="w-full py-3 rounded-xl bg-gray-100 text-gray-700 font-medium hover:bg-gray-200 flex items-center justify-center gap-2">
          ...
          {mostrarRevisao && erros.length > 0 && (
            <div className="space-y-4">
              ...
              {resultados.map((resultado, index) => {
                ...
                return (
                  <div key={resultado.questaoId} className={`bg-white rounded-xl border-2 overflow-hidden ${resultado.correta ? 'border-emerald-200' : 'border-red-200'}`}>
                    <button
                      onClick={() => setQuestaoExpandida(isExpandida ? null : resultado.questaoId)}
                      className="w-full p-4 flex items-center justify-between hover:bg-gray-50"
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${resultado.correta ? 'bg-emerald-100' : 'bg-red-100'}`}>
                          {resultado.correta ? <CheckCircle className="w-5 h-5 text-emerald-600" /> : <XCircle className="w-5 h-5 text-red-600" />}
                        </div>
                      ...
                      {isExpandida ? <ChevronUp className="w-5 h-5 text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}
                    </button>
        ...
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="flex gap-1">
              {[0, 1, 2, 3, 4].map(i => (
                <div key={i} className={`w-3 h-3 rounded-full ${i < questaoAtual ? 'bg-white' : i === questaoAtual ? 'bg-white animate-pulse' : 'bg-white/30'}`} />
              ))}
            </div>
      ...
      <main className="px-4 pb-8">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-3xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                ...
                questao.dificuldade === 'dificil' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
              }`}>
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl border-2 text-left transition-all ${
                      respondendo ? 'border-gray-200 bg-gray-50 cursor-not-allowed opacity-50' : 'border-gray-200 hover:border-amber-400 hover:bg-amber-50 active:bg-amber-100'
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      <span className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-sm font-bold text-gray-600 shrink-0">{letra}</span>
          ...
          <div className="mt-4 h-2 bg-white/30 rounded-full overflow-hidden">
            <div className={`h-full transition-all duration-1000 ${tempoRestante > 20 ? 'bg-emerald-400' : tempoRestante > 10 ? 'bg-amber-400' : 'bg-red-400'}`} style={{ width: `${(tempoRestante / 30) * 100}%` }} />
          </div>
        </div>
      </main>
    </div>
