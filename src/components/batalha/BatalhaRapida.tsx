...
export function BatalhaRapida({ userId }: BatalhaRapidaProps) {
  ...
  const confirmarResposta = useCallback(() => {
    ...
    if (acertou) {
      ...
      supabase.rpc('registrar_erro', {
        p_user_id: userId,
        p_questao_id: questao.id,
        p_resposta_usuario: respostaSelecionada
      }).catch(console.error);
    }
  }, [respostaConfirmada, questoes, questaoAtual, respostaSelecionada, tempoRestante, userId, supabase]);
    ...
    return (
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-slate-700/50 p-8">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-blue-500 to-indigo-600 blur-3xl"></div>
        </div>
        <div className="relative z-10 text-center">
          <div className="w-24 h-24 mx-auto rounded-3xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-2xl mb-6">
          ...
          <div className="flex items-center justify-center gap-6 mb-8">
            <div className="text-center"><div className="w-16 h-16 rounded-2xl bg-slate-700/50 flex items-center justify-center mb-2"><Timer className="w-8 h-8 text-slate-300" /></div><p className="text-xs text-slate-500">{TEMPO_POR_QUESTAO}s por questão</p></div>
          ...
          <button onClick={iniciarBatalha} disabled={loading} className="px-10 py-4 rounded-2xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-bold text-lg shadow-lg hover:scale-105 transition-all disabled:opacity-50">
            {loading ? <span className="flex items-center gap-2"><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>Carregando...</span> : <span className="flex items-center gap-2"><Swords className="w-5 h-5" />Iniciar Batalha<Sparkles className="w-5 h-5" /></span>}
          </button>
        </div>
      </div>
    );
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="text-slate-400 text-sm">Questão</span>
          ...
          <div className="flex items-center gap-3">
            <Timer className={`w-6 h-6 ${tempoClasse}`} />
            <span className={`text-3xl font-mono font-bold ${tempoClasse}`}>{tempoRestante}s</span>
          </div>
        </div>
        <div className="relative h-2 bg-slate-700/50 rounded-full overflow-hidden">
          <div className={`absolute inset-y-0 left-0 ${tempoRestante <= 10 ? 'bg-red-500' : tempoRestante <= 20 ? 'bg-yellow-500' : 'bg-emerald-500'} rounded-full transition-all duration-1000`} style={{ width: `${tempoPercentual}%` }}></div>
        </div>
        <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-6">
          <p className="text-lg text-white leading-relaxed">{questao.enunciado}</p>
        </div>
        <div className="space-y-3">
          {alternativas.map(({ letra, texto }) => {
            ...
            if (respostaConfirmada) {
              if (correta) classes = 'border-emerald-500 bg-emerald-500/20';
              else if (selecionada) classes = 'border-red-500 bg-red-500/20';
            } else if (selecionada) {
            ...
            return (
              <button key={letra} onClick={() => !respostaConfirmada && setRespostaSelecionada(letra)} disabled={respostaConfirmada} className={`w-full p-4 rounded-xl border-2 text-left transition-all ${classes}`}>
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold shrink-0 ${respostaConfirmada && correta ? 'bg-emerald-500 text-white' : respostaConfirmada && selecionada ? 'bg-red-500 text-white' : selecionada ? 'bg-blue-500 text-white' : 'bg-slate-700/50 text-slate-400'}`}>
        ...
        <div className="flex gap-4">
          {!respostaConfirmada ? (
            <button onClick={confirmarResposta} disabled={!respostaSelecionada} className={`flex-1 py-4 rounded-xl font-bold text-white transition-all ${respostaSelecionada ? 'bg-gradient-to-r from-blue-500 to-indigo-600 hover:opacity-90' : 'bg-slate-700/50 text-slate-500 cursor-not-allowed'}`}>Confirmar</button>
          ) : (
            <button onClick={proximaQuestao} className="flex-1 py-4 rounded-xl font-bold text-white bg-gradient-to-r from-blue-500 to-indigo-600 hover:opacity-90 transition-all flex items-center justify-center gap-2">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border border-slate-700/50 p-8">
        <div className="relative z-10 text-center">
          ...
          <div className="grid grid-cols-2 gap-6 max-w-sm mx-auto my-8">
            <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700/50"><p className="text-4xl font-black text-white">{acertos}/{QUESTOES_POR_BATALHA}</p><p className="text-slate-400 text-sm">Acertos</p></div>
            <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700/50"><p className="text-4xl font-black text-yellow-400">+{xpGanho}</p><p className="text-slate-400 text-sm">XP Ganho</p></div>
          </div>
          <div className="w-full max-w-xs mx-auto mb-8">
            <div className="flex justify-between text-sm text-slate-400 mb-2"><span>Taxa de Acerto</span><span>{percentual}%</span></div>
            <div className="h-4 bg-slate-700/50 rounded-full overflow-hidden"><div className={`h-full ${percentual >= 80 ? 'bg-emerald-500' : percentual >= 60 ? 'bg-blue-500' : percentual >= 40 ? 'bg-yellow-500' : 'bg-red-500'} rounded-full`} style={{ width: `${percentual}%` }}></div></div>
          </div>
          <div className="flex gap-4 justify-center">
            <button onClick={reiniciar} className="px-6 py-3 rounded-xl bg-slate-700/50 text-slate-300 font-medium hover:bg-slate-600/50 transition-all flex items-center gap-2"><RotateCcw className="w-5 h-5" />Nova Batalha</button>
          </div>
        </div>
      </div>
    );
